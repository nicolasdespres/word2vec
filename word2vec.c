...
char train_file[MAX_ARG_STRING], output_file[MAX_ARG_STRING],
    output_classifier[MAX_ARG_STRING];
char save_vocab_file[MAX_ARG_STRING], read_vocab_file[MAX_ARG_STRING];
struct vocab_word *vocab;
int binary = 0, cbow = 1, debug_mode = 2, window = 5, min_count =
    5, num_threads = 12, min_reduce = 1;
int *vocab_hash;
long long vocab_max_size = 1000, vocab_size = 0, layer1_size = 100;
long long train_words = 0, word_count_actual = 0, iter = 5, file_size =
    0, classes = 0;
...
void *TrainModelThread(void *id)
{
  ...
  while (1) {
    if (word_count - last_word_count > 10000) {
      ...
      alpha =
          starting_alpha * (1 -
                            word_count_actual / (real) (iter * train_words +
                                                        1));
    ...
    if (sentence_length == 0) {
      while (1) {
        ...
        if (sample > 0) {
          real ran =
              (sqrt(vocab[word].cn / (sample * train_words)) +
  ...
  if (classes == 0) {
    ...
    if (output_classifier && hs) {
      ...
      for (a = 0; a < vocab_size; a++) {
        if (binary) {
          for (b = 0; b < layer1_size; b++)
            fwrite(&syn1[a * layer1_size + b], sizeof(real), 1, fhs);
        } else {
          for (b = 0; b < layer1_size; b++)
            fprintf(fhs, "%lf ", syn1[a * layer1_size + b]);
          fprintf(fhs, "\n");
        }
      }
      fclose(fhs);
...
int main(int argc, char **argv)
{
  ...
  if (argc == 1) {
    ...
    printf
        ("\t\tOutput the classifier (one vector or the hierarchy); default is 0 (off)\n");
  ...
  if ((i = ArgPos((char *)"-output-classifier", argc, argv)) > 0)
    strcpy(output_classifier, argv[i + 1]);
